...
这个 Worker 负责保存浏览器推送订阅、按分钟扫描待发送提醒，在用户逐次明确同意后把压缩睡眠截图转给 Workers AI 做一次结构化识别，并为规则归因提供可选的自然语言解释。
...
- UTC 执行时间
- 匿名设备与网络指纹的当日 AI 调用次数（均为 SHA-256 摘要，不保存原始 IP）

推送端点不接收提醒标题和描述，也不接收睡眠、症状、情绪、归因、实验结果或补剂记录。解释端点仅在一次请求期间接收白名单枚举摘要和问题，但不持久化请求或回答。订阅记录最长保留 60 天，单次待发送索引最长保留 48 小时；无效订阅会在推送服务返回 404/410 时删除。
...
`POST /explain` 使用 `@cf/meta/llama-3.1-8b-instruct-fast`。请求先被裁剪为分型、负担分档、归因 / 行动 ID、症状标签与一个不超过 160 字的问题，不接收精确时间、剂量、分数、截图、备注或历史。用户问题按不可信输入隔离；模型回答若含数字会被拒绝。请求与回答均不写入 KV；每台匿名设备每日最多调用 3 次。
